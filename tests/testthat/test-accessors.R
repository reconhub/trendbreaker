test_that("trendbreaker accessors", {
  ...
  nms <- c(colnames(dat), "pred", "lower-pi", "upper-pi", "observed", "outlier", "classification")
  expect_equal(names(get_results(res)), nms)

  outliers <- get_outliers(res)
  expect_equal(nrow(outliers), 0)
  expect_equal(ncol(outliers), 8)

  pred <- predict(res, dat)
  nms <- c(colnames(dat), "pred", "lower-pi", "upper-pi", "observed")
